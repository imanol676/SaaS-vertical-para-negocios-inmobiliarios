      ...
      <nav className="px-3 pb-6" aria-label="Navegación principal">
        <ul className="space-y-1">
          {demoNavItems.map((item) => {
            // A simple logic for active link because we don't have deep sub-routes in demo yet
            const isActive = pathname === item.href || pathname.startsWith(item.href + "/") && item.href !== "/demo";
